...
void chooseMode(){
  // IMPLEMENT
//  int buttonValue = digitalRead(BUTTON_MODE_PIN);
...
//    Serial.println(sensorValue);
    //IMPLEMENT
    if(analogSensorVal > 3 && analogSensorVal < 20){
    tone(BUZZER_PIN, testNote[0]);  
    } else if (analogSensorVal > 80 && analogSensorVal < 100){
    tone(BUZZER_PIN, testNote[1]);   
    } else if (analogSensorVal > 900 && analogSensorVal < 920){
    tone(BUZZER_PIN, testNote[2]);   
    } else if (analogSensorVal > 925 && analogSensorVal < 940){
    tone(BUZZER_PIN, testNote[3]);   
    } else if (analogSensorVal > 1000 && analogSensorVal < 1030){
    tone(BUZZER_PIN, testNote[4]);   
    }
...
void record()
{
  // IMPLEMENT
     ...
     if(analogSensorVal > 3 && analogSensorVal < 20){
    tone(BUZZER_PIN, testNote[0]); 
    notes[noteIndex] = testNote[0]; 
    noteIndex = (noteIndex + 1) % 16;
    delay(500);
    } else if (analogSensorVal > 80 && analogSensorVal < 100){
    tone(BUZZER_PIN, testNote[1]);   
    notes[noteIndex] = testNote[1]; 
    noteIndex = (noteIndex + 1) % 16;
    delay(500);
    } else if (analogSensorVal > 900 && analogSensorVal < 920){
    tone(BUZZER_PIN, testNote[2]);   
    notes[noteIndex] = testNote[2]; 
    noteIndex = (noteIndex + 1) % 16;
    delay(500);
    } else if (analogSensorVal > 925 && analogSensorVal < 940){
    tone(BUZZER_PIN, testNote[3]);
        notes[noteIndex] = testNote[3]; 
    noteIndex = (noteIndex + 1) % 16; 
    delay(500);  
    } else if (analogSensorVal > 1000 && analogSensorVal < 1030){
    tone(BUZZER_PIN, testNote[4]); 
        notes[noteIndex] = testNote[4]; 
    noteIndex = (noteIndex + 1) % 16;  
    delay(500);
    }
